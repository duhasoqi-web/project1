...
const defaultSupplier: SupplierData = {
  name: "", date: "", method: "", price: "", currency: "", notes: "",
};

const defaultSupplierNames = ["المكتبة الوطنية", "دار الشروق", "مكتبة الكتب"];

export default function Supplier({ formData, updateData }: SupplierProps) {
  const supplier: SupplierData = formData.supplier_data || defaultSupplier;
  const [customSuppliers, setCustomSuppliers] = useState<string[]>([]);

  const allSuppliers = [...defaultSupplierNames, ...customSuppliers];

  const updateField = (key: keyof SupplierData, value: string) => {
    updateData("supplier_data", { ...supplier, [key]: value });
  };

  const addSupplierOption = () => {
    const newSupplier = prompt("أدخل اسم المزود الجديد:");
    if (!newSupplier) return;
    if (allSuppliers.includes(newSupplier)) {
  ...
  return (
    <div className="animate-fade-in space-y-5">
      <h3 className="text-lg font-semibold text-foreground">بيانات المزوّد</h3>
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <Button type="button" variant="outline" size="icon" onClick={addSupplierOption} title="أضف مزود جديد">
        ...
        <div className="space-y-2">
          <Label htmlFor="supplierDate">تاريخ التزويد</Label>
          <Input id="supplierDate" type="date" value={supplier.date} onChange={(e) => updateField("date", e.target.value)} />
        </div>
      ...
      <div className="space-y-2">
        ...
        <Textarea id="supplierNotes" rows={3} placeholder="ملاحظات إضافية..." value={supplier.notes} onChange={(e) => updateField("notes", e.target.value)} />
      </div>
    </div>
  );
}
